...
void* SDE_MemoryPool::Allocate(size_t nSize)
{
	if (nSize == 0)
	{
		return nullptr;
	...
	else
	{
		...
		uintptr_t nBlockCount = s_nChunkSize / pChunkInfo->nChunkSize;
		for (uintptr_t i = 0; i < nBlockCount - 1; i++)
		{
...
void SDE_MemoryPool::Free(void* pObject, size_t nSize)
{
	if (nSize == 0)
	{
		return;
...
SDE_MemoryPool g_memoryPool = SDE_MemoryPool();